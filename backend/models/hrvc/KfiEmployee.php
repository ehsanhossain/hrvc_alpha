    ...
    public static function kfiEmployee($kfiId)
    {
        ...
        if (isset($kfiEmployee) && count($kfiEmployee) > 0) {
            foreach ($kfiEmployee as $ke) :
                if ($ke["picture"] != "") {
                    if (Path::isLocal()) {
                        $filePath = Yii::getAlias('@frontend') . '/web/' . $ke["picture"];
                        $employee[$ke["employeeId"]] = file_exists($filePath) ? $ke["picture"] : $img;
                    } else {
                        $url = Path::frontendUrl() . $ke["picture"];
                        $headers = @get_headers($url);
                        $employee[$ke["employeeId"]] = ($headers && strpos($headers[0], '200') !== false) ? $ke["picture"] : $img;
                    }
                } else {
        ...
        if (isset($kfiEmployee) && count($kfiEmployee) > 0) {
            foreach ($kfiEmployee as $ke) :
                if ($ke["picture"] != "") {
                    if (Path::isLocal()) {
                        $filePath = Yii::getAlias('@frontend') . '/web/' . $ke["picture"];
                        $employee[$ke["employeeId"]]["picture"] = file_exists($filePath) ? $ke["picture"] : $img;
                    } else {
                        $url = Path::frontendUrl() . $ke["picture"];
                        $headers = @get_headers($url);
                        $employee[$ke["employeeId"]]["picture"] = ($headers && strpos($headers[0], '200') !== false) ? $ke["picture"] : $img;
                    }
                } else {
