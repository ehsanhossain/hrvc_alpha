    ...
    <?php foreach ($rowData as $key => $list_month) { ?>
        <table class="table-item item-data me-1 table-data-pl ">
            <thead>
                ...
                <tr class="row-title-1">
                    <th class="px-1 data-pl content-cp show" style="min-width:68px">
            ...
            <tbody>
                ...
                ?>
                    <tr class="upper m-<?php echo $i; ?>">
                        <td class="data-pl content-cp show"><input type="text" value="<?php echo $row_detail['actual_amount']; ?>" onchange="updateData('<?php echo $row_detail['account_id']; ?>','1','<?php echo $row_detail['month']; ?>','<?php echo $row_detail['year']; ?>',this.value,$(this))" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" placeholder="0"></td>
                        <td class="data-pl content-ct"><input type="text" value="<?php echo $row_detail['target_amount']; ?>" onchange="updateData('<?php echo $row_detail['account_id']; ?>','2','<?php echo $row_detail['month']; ?>','<?php echo $row_detail['year']; ?>',this.value,$(this))" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" placeholder="0"></td>
                        <td class="data-pl content-nt"><input type="text" value="<?php echo $row_detail['next_target_amount']; ?>" onchange="updateData('<?php echo $row_detail['account_id']; ?>','3','<?php echo $row_detail['month']; ?>','<?php echo $row_detail['year']; ?>',this.value,$(this))" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" placeholder="0"></td>
                    </tr>
                <?php
                    $i++;
                }
