...
foreach ($list_breakdown_data[$breakdown_index]['data'] as $value) {
    $array_data['monthName'][] = $value['monthName'] . $value['year'];
    $array_data['last_actual_amount'][] = round(changeCurrencyValue($currencyDefaul, $currencyFocus, $value['last_actual_amount']) / $rate, 2);
    $array_data['actual_amount'][] = round(changeCurrencyValue($currencyDefaul, $currencyFocus, $value['actual_amount']) / $rate, 2);
    $array_data['target_amount'][] = round(changeCurrencyValue($currencyDefaul, $currencyFocus, $value['target_amount']) / $rate, 2);
    $array_data['next_target_amount'][] = round(changeCurrencyValue($currencyDefaul, $currencyFocus, $value['next_target_amount']) / $rate, 2);
}
