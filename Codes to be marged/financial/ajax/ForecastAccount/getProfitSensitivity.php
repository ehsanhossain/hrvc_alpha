...
$currency_filter = mysqli_real_escape_string($connection, $_POST['currency']);

// ----------------------------------------------------------------------------------------------
$sql_currency1 = "SELECT c.* FROM branch b
LEFT JOIN tbl_currency c ON c.currency_id = b.currency_default 
WHERE b.branchId = '$branchId'";
$rs_currency1 = mysqli_query($connection, $sql_currency1) or die($connection->error);
$currency_default1 = mysqli_fetch_assoc($rs_currency1);

$FirstDivide = $currency_default1['divide_value'];
$FirstExchange = $currency_default1['exchange_rate'];

// ----------------------------------------------------------------------------------------------

$sql_currency2 = "SELECT c.* FROM tbl_currency c
WHERE c.currency_id = '$currency_filter'";
$rs_currency2 = mysqli_query($connection, $sql_currency2) or die($connection->error);
$currency_default2 = mysqli_fetch_assoc($rs_currency2);

$SecondDivide = $currency_default2['divide_value'];
$SecondExchange = $currency_default2['exchange_rate'];
...
for ($breakdown_id = 1; $breakdown_id <= 8; $breakdown_id++) {
    ...
    foreach ($list_month as $rowMonth) {
        ...
        $list_data[count($list_data) - 1]['data'][] = array(
            ...
            "month" => $month,
            "year" => $year,
            "last_actual_amount" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $last_act),
            "actual_amount" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $actual_amount),
            "target_amount" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $target_amount),
            "next_target_amount" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $next_target_amount),
            "accum_last_act" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $accum_last_act),
            "accum_act" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $accum_act),
            "accum_tar" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $accum_tar),
            "accum_next_tar" => currencyExchange($FirstExchange, $FirstDivide, $SecondExchange, $SecondDivide, $accum_next_tar)
        );
    }
